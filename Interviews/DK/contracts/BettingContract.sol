...
contract BettingContract is Helpers {
    ...
    function createContest(uint256 startTime, uint256 endTime, uint256 maxBets) external onlyMod() {
    ...
    function bet(uint256 contestId, bool homeWin) external payable onlyValidBetTime(contestId) {
        ...
        require(msg.value > 100000000000000000, "Minimun bet is 0.1ETH");
    ...
    function beginContest(uint256 contestId) external onlyMod onlyValidBetTime(contestId) {
    ...
    function endContest(uint256 contestId, bool homeWin) external onlyMod {
    ...
    function getPayout(uint contestId) external  {
    ...
    function getContestStatus(uint contestId) external view returns(Status) {
        return(contests[contestId].status);
    }

    function getContestInfos(uint contestId) external view onlyValidBetTime(contestId) returns(Contest) {
